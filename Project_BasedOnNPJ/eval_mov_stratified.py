...
df = pd.read_csv(dataset_root + "/out/mov_res_folds.csv", sep="\t")
...
for mode in ["pd_vs_dd", "pd_vs_hc"]:
    _, y, _, idxs = get_dataset(type="movement", mode=mode, return_idxs=True)
...
df_mean.to_csv(f"{dataset_root}/out/mov_res_stratified.csv", index=False, sep="\t")
print("done")
