...
experiments = experiments[experiments["data"] == "questionnaire"]
...
df_folds.to_csv(dataset_root + "/out/quest_res_folds.csv", index=False, sep="\t")

params_folds = [params_list[idx] for idx in idx_max]
with open(dataset_root + "/out/best_params_quest.json", "w") as f:
...
df_mean.to_csv(dataset_root + "/out/quest_res.csv", index=False, sep="\t")
